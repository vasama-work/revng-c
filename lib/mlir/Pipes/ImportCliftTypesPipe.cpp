...
struct MLIRTraits {
  ...
  static std::optional<Location> getLocation(mlir::Operation *const O) {
    auto Loc = mlir::dyn_cast_or_null<mlir::FileLineColLoc>(O->getLoc());

    if (not Loc)
      return std::nullopt;

    return Location::fromString(Loc.getFilename().getValue());
  }

  static TupleTree<efa::FunctionMetadata> extractFunctionMetadata(LLVMFuncOp F) {
  ...
  static const model::Function *getModelFunction(const model::Binary &Binary,
                                                 LLVMFuncOp F) {
    auto const MaybeFunctionName = getFunctionName(
      mlir::cast<mlir::FunctionOpInterface>(F.getOperation()));
  ...
  void run(const pipeline::ExecutionContext &Ctx,
    ...
    const auto importFunctionPrototype = [&](const model::Type &ModelType) {
      ...
      fprintf(stderr,
        "import %lu '%s'\n",
        ModelType.ID(),
        std::string(ModelType.CustomName()).c_str());
      ...
      const auto CliftType = revng::getUnqualifiedType(Context, ModelType);
