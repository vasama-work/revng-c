...
EnumAttr::verify(::llvm::function_ref<::mlir::InFlightDiagnostic()> emitError,
  ...
  const uint64_t Width = PrimitiveType.getSize() * 8;
  ...
  case PrimitiveKind::UnsignedKind:
    MaxValue = llvm::APInt::getMaxValue(Width).getZExtValue();
    break;
  case PrimitiveKind::SignedKind:
    MinValue = llvm::APInt::getSignedMinValue(Width).getSExtValue();
    MaxValue = llvm::APInt::getSignedMaxValue(Width).getSExtValue();
    IsSigned = true;
    break;
  default:
