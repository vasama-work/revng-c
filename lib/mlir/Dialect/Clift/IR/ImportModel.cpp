...
private:
  ...
  getScalarTupleType(const model::RawFunctionType &ModelType) {
    ...
    for (const model::NamedTypedRegister &Register : ModelType.ReturnValues()) {
      auto const RegisterType = rc_recur getQualifiedValueType(Register.Type());
      if (not RegisterType)
        rc_return nullptr;

      auto const Attribute = make<ElementAttr>(RegisterType, Register.name());
      if (not Attribute)
        rc_return nullptr;
  ...
  getTypeAttribute(const model::UnionType &ModelType,
    ...
    for (const model::UnionField &Field : ModelType.Fields()) {
      ...
      if (not FieldType)
        rc_return nullptr;
      auto const Attribute = make<clift::FieldAttr>(0, FieldType, Field.name());
      if (not Attribute)
        rc_return nullptr;
      Fields.push_back(Attribute);
    }
